<template>
  ...
  <div class="mt-5">
    <ul class="nav" id="myTab" role="tablist" style="position: sticky; top: 0;background-color: #fff;z-index: 999;">
      ...
      <li class="nav-item" role="presentation" @click="activeTab = '5'" v-if="checkuser == user_id" >
        <a
          class="nav-link"
      ...
      <li class="nav-item" role="presentation" @click="activeTab = '6',scrollToDownforStages()" v-if="checkuser == user_id" >
        <a
          class="nav-link"
...
onMounted(() => {
  getAllComments();
  scrollToTop();
  checkuser.value =JSON.parse(localStorage.getItem('project'));
