<template>
  <div>
          ...
          <div class="modal-body">
            <div class="input-group">
              <div class="input-group-prepend">
                ...
                <div @click="rejectProject()" class="btn btn-primary">
                  تأكيد العملية
                </div>
              </div>
              <textarea
            ...
            <div class="text-center">
              <span class="text-danger fs-6">{{ rejected }}</span>
            </div>
          </div>
        </div>
      </div>
    ...
    <div class="loading w-100 mt-5 pt-5" v-if="allProjects.length == 0">
      <div class="text-center w-100 tex-white">
        <span class="fs-3 d-inline-block tex-white" style="color: white"
          >جارى التحميل...</span
        >
      </div>
      <div class="w-100 text-center">
        <span class="d-inline-block">
          <img
            :src="require('@/assets/DoubleRing.gif')"
                  ...
                  <div class="row my-3">
                    ...
                    >
                      ...
                      <div class="card-content">
                        ...
                        <span class="card-content-text mt-4"
                          >العروض المستلمة</span
                        >
                      </div>
                      <div>
                    ...
                    >
                      ...
                      <div class="text-center">
                        <span
                          v-for="myP in project.electronic_service"
                          :key="myP"
                        >
                          {{ myP.name }}
                        </span>
                    ...
                    <div
                      class="d-xl-flex d-lg-flex d-md-flex d-sm-flex d-grid justify-content-center py-3 text-center"
                    >
                      <div
                        class="col-xl-5 col-lg-5 col-md-6 col-sm-6 col-12 pb-3"
                      >
                        <p>صورة صك الملكية</p>
                        <img
                          :src="`https://test.m-aboelela.online/projects/${project.id}/${project.owner_id?.name}`"
                          alt="صك الملكية"
                          class="w-25 rounded-5 titel_deed"
                        />
                      </div>
                      <div
                        class="col-xl-5 col-lg-5 col-md-6 col-sm-6 col-12 pb-3"
                      >
                        ...
                        <img
                          :src="
                            'https://test.m-aboelela.online/projects/' +
                            project.id +
                            '/' +
                            project.title_deed?.name
                          "
                          alt="هوية المالك"
                          class="w-25 titel_deed rounded-5"
                        />
                      </div>
                      <!--
<div class="col-3">
                    ...
                    <div
                      class="project_status justify-content-center d-xl-flex d-lg-flex d-md-flex d-sm-flex d-grid"
                    >
                      <button
                        :disabled="
                          project.state == 0
                            ...
                            : true
                        "
                        @click="acceptProject(project, index)"
                        class="mt-2 rounded-3 btn btn-success px-5 text-white"
                      >
                        موافقة
                      </button>
                      <div class="spreator me-2"></div>
                      <button
                        :disabled="
                          project.state == 0
                            ? false
                        ...
                        "
                        type="button"
                        class="mt-2 btn btn-primary rounded-3 px-5"
                        data-toggle="modal"
                      ...
                      <button
                        type="button"
                        class="btn btn-danger rounded-3 delete-button px-5 mt-2"
                    ...
                    <div>
                      <div
                        v-if="loading == index"
                        class="text-center pt-3 d-flex justify-content-center"
                      >
                        <span class="fw-bold text-info fs-5"
                          >...جارى تنفيذ العملية</span
                    ...
                    <div class="text-center" v-if="registerd == index">
  ...
  setup() {
    const logOut = () => {
      if (
        ...
        localStorage.getItem("office_notifications")
      ) {
    ...
    const getAllProjects = async () => {
      await axios
        .get("https://test.m-aboelela.online/api/admin/projects/index", {
          headers: {
    ...
    const acceptProject = async (project, index) => {
      loading.value = index;
      await axios
        .post(
          `https://test.m-aboelela.online/api/admin/projects/approve?project_id=${project.id}`,
          {
        ...
        .then((res) => {
          if (res.status == 200) {
            getAllProjects();
            loading.value = -1;
            registerd.value = index;
            registerd.value = "تم الموافقة على المشروع";
            setTimeout(() => {
              registerd.value = "";
            }, 3000);
          }
        ...
        .catch((err) => {
          registerd.value = index;
          loading.value = -1;
          registerd.value = "فشل اجراء العملية";
          console.log(err);
          setTimeout(() => {
            registerd.value = "";
          }, 3000);
    ...
    const filteredProjects = computed(() =>
      allProjects.value.filter((project) => project.state == 1)
    );
    onMounted(() => {
      getAllProjects();
    });
    const rejectProject = async () => {
      await axios
        .post(
          `https://test.m-aboelela.online/api/admin/projects/reject?project_id=${projectId.value}`,
          {
    ...
    const loading = ref(-1);
    const registerd = ref("");
    const deleteProject = async (project, index) => {
      loading.value = index;
      await axios
        .post(
          `https://test.m-aboelela.online/api/admin/projects/delete?project_id=${project.id}`,
          {
            project_id: project.id,
          },
          {
        ...
        .then((res) => {
          if (res.status == 200) {
            registerd.value = index;
            registerd.value = "تم حذف المشروع";
            getAllProjects();
            loading.value = -1;
          }
        })
